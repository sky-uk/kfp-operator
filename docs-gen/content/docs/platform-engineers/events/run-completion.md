...
linkTitle: "Run Completion Events"
description: "Technical specification for pipeline run completion event schemas and integration"
weight: 40
---

# Run Completion Events Reference

This section provides the canonical technical specification for run completion events in the KFP Operator, including event schemas, integration patterns, and system behavior.

Run completion events are created by a [`Provider`](../configuration/providers/overview) to signal the completion of a pipeline run, and is used to trigger downstream processes.
...
Make sure to provide an event bus for the eventsource and the sensor to connect to.
