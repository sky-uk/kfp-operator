...
| `spec.podTemplateVolumes[]`      | A list of [volumes](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.34/#volume-v1-core) appended to this provider's service pod. Every volume must be referenced by a matching `spec.podTemplateVolumeMounts[]` entry to be usable by the container. | ```- name: kfp-sa-token```<br>```  projected:```<br>```    sources:```<br>```    - serviceAccountToken:```<br>```        path: token```<br>```        audience: pipelines.kubeflow.org``` |
| `spec.podTemplateVolumeMounts[]` | A list of [volume mounts](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.34/#volumemount-v1-core) appended to this provider's service container. Each `name` must reference a volume declared in `spec.podTemplateVolumes[]`, otherwise the resource is rejected at admission. | ```- name: kfp-sa-token```<br>```  mountPath: /var/run/secrets/kfp```<br>```  readOnly: true``` |
...
| `pscNetworkAttachment`                  | The network attachment for [Private Service Connect](https://docs.cloud.google.com/vpc/docs/private-service-connect) to be used when submitting a job |
