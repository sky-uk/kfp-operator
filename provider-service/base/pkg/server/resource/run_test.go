...
var _ = Describe("Run", Ordered, func() {
	...
	BeforeEach(func() {
		mockProvider = MockRunProvider{}
		underTest = Run{Provider: &mockProvider}
	})
	...
	Context("Create", func() {
		When("valid json passed, and provider returns success", func() {
			It("returns the id of the resource", func() {
				...
				jsonRun, err := json.Marshal(run)

				Expect(err).ToNot(HaveOccurred())

				id := "some-id"

				mockProvider.On("CreateRun", run).Return(id, nil)

				response, err := underTest.Create(jsonRun)
		...
		When("provider errors", func() {
			It("errors", func() {
				...
				mockProvider.On("CreateRun", run).Return("", expectedErr)
