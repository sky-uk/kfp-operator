...
var _ = Context("Resource Workflows", Serial, func() {
	...
	DescribeTable("Creation Workflow", AssertWorkflow[*pipelinesv1.RunConfiguration],
		Entry("Creation succeeds",
			newRunConfiguration,
			SucceedCreation[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructCreationWorkflow,
		), Entry("Creation fails",
			newRunConfiguration,
			FailCreation[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructCreationWorkflow,
		), Entry("Updating succeeds",
			newRunConfiguration,
			SucceedUpdating[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructUpdateWorkflow,
		), Entry("Updating fails",
			newRunConfiguration,
			FailUpdating[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructUpdateWorkflow,
		), Entry("Deletion succeeds",
			newRunConfiguration,
			SucceedDeletion[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructDeletionWorkflow,
		), Entry("Deletion fails",
			newRunConfiguration,
			FailDeletion[*pipelinesv1.RunConfiguration],
			workflowFactory.ConstructDeletionWorkflow,
		),
	)
})
