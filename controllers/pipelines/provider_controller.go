...
func (r *ProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if providerServiceDeployment == nil {
		providerServiceDeployment = constructProviderServiceDeployment(providerServiceName, req.Namespace, provider.GetName(), r.Config.ContainerRegistry, provider.Spec.Type)
...
func constructProviderServiceDeployment(name, namespace, providerName, containerRegistry string, providerType pipelinesv1.ProviderType) *appsv1.Deployment {
					...
					Containers: []v1.Container{
						{
							...
							Image: fmt.Sprintf("%s/kfp-operator-%s-provider-service", containerRegistry, providerType),
						},
					},
				},
			},
