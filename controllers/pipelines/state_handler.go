...
func (st *StateHandler[R]) stateTransition(ctx context.Context, provider pipelinesv1.Provider, resource R) (commands []Command) {
	...
	if resourceProvider != "" && resourceProvider != provider.Name {
		...
		case apis.Creating:
			commands = st.onCreating(ctx, resource, st.WorkflowRepository.GetByLabels(ctx, foo.CommonWorkflowLabels(resource)))
		...
		case apis.Updating:
			commands = st.onUpdating(ctx, resource, st.WorkflowRepository.GetByLabels(ctx, foo.CommonWorkflowLabels(resource)))
		case apis.Deleting:
			commands = st.onDeleting(ctx, resource, st.WorkflowRepository.GetByLabels(ctx, foo.CommonWorkflowLabels(resource)))
		case apis.Deleted:
		default:
...
func (st *StateHandler[R]) onUnknown(ctx context.Context, provider pipelinesv1.Provider, resource R) []Command {
	...
	if resource.GetStatus().Provider.Id != "" {
		...
		if err != nil {
			failureMessage := foo.WorkflowConstants.ConstructionFailedError
...
func (st StateHandler[R]) setStateIfProviderFinished(ctx context.Context, status pipelinesv1.Status, workflows []argo.Workflow, states IdVerifier) []Command {
	...
	statusFromProviderOutput := func(workflow *argo.Workflow) *SetStatus {
		...
		result, err := workflowutil.GetWorkflowOutput(workflow, foo.WorkflowConstants.ProviderOutputParameterName)
		provider := workflowutil.GetWorkflowParameter(workflow, foo.WorkflowConstants.ProviderNameParameterName)
