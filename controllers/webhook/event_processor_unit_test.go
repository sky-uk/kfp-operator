...
func schemeWithCRDs() *runtime.Scheme {
	...
	scheme.AddKnownTypes(groupVersion, &v1alpha6.RunConfiguration{}, &v1alpha6.Run{})

	metav1.AddToGroupVersion(scheme, groupVersion)
	return scheme
}

func checkOutputArtifacts(outputArtifacts []v1alpha6.OutputArtifact, expectedArtifacts []v1alpha6.OutputArtifact) {
...
var _ = Context("ToRunCompletionEvent", func() {
	...
	When("given valid runCompletionEventData", func() {
		It("converts to a runCompletionEvent with filtered artifacts", func() {
			rc := v1alpha6.RandomRunConfiguration()
			...
			stubbedFilterFunc := func(_ []common.PipelineComponent, _ []v1alpha6.OutputArtifact) []common.Artifact {
...
var _ = Context("filter", func() {
	...
	baseOutputArtifact := v1alpha6.OutputArtifact{
		Name: "outputArtifactName",
		Path: v1alpha6.ArtifactPath{
			Locator: v1alpha6.ArtifactLocator{
	...
	When("all fields match", func() {
		It("returns the matching artifact", func() {
			result := filterByResourceArtifacts([]common.PipelineComponent{basePipelineComponent}, []v1alpha6.OutputArtifact{baseOutputArtifact})
	...
	When("component name is mismatched", func() {
		It("returns no artifact", func() {
			...
			result := filterByResourceArtifacts([]common.PipelineComponent{basePipelineComponent}, []v1alpha6.OutputArtifact{nonMatchingComponent})
	...
	When("artifact name is mismatched", func() {
		It("returns no artifact", func() {
			...
			result := filterByResourceArtifacts([]common.PipelineComponent{basePipelineComponent}, []v1alpha6.OutputArtifact{nonMatchingArtifactName})
			Expect(result).To(BeEmpty())
		})
	})
	...
	When("artifact is missing a uri", func() {
		It("returns no artifact", func() {
			...
			result := filterByResourceArtifacts([]common.PipelineComponent{missingArtifact}, []v1alpha6.OutputArtifact{baseOutputArtifact})
			Expect(result).To(BeEmpty())
		})
	})
	...
	When("output artifact filter evaluates false on artifact metadata", func() {
		It("returns no artifact", func() {
			...
			result := filterByResourceArtifacts([]common.PipelineComponent{basePipelineComponent}, []v1alpha6.OutputArtifact{nonMatchingFilter})
			Expect(result).To(BeEmpty())
		})
	})
	...
	When("output artifact locator index is more than component artifacts length", func() {
		It("returns no artifact", func() {
			...
			result := filterByResourceArtifacts([]common.PipelineComponent{basePipelineComponent}, []v1alpha6.OutputArtifact{invalidIndex})
			Expect(result).To(BeEmpty())
		})
	})
})
